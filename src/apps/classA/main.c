...
static void PrepareTxFrame(uint8_t port) {
	...
	uint8_t ret;
	uint16_t bat;
	uint8_t humi = 55; // Humidity
	switch (port) {
	//https://mydevices.com/cayenne/docs/lora/#lora-cayenne-low-power-payload
		//cayenne LPP Temp
	case 2: {
// 2 bytes For Cayenne
//		AppData[0] = 0x01; // Channel
//		AppData[1] = 0x67; // Type temperature
#if 1
		LIS3DH_GetTempRaw(&tempr); //only tempr changed value
		tempr = tempr + 25; // temprature should be calibration  in a right temp for every device
#else
				tempr = randr( 15, 22);
#endif
//		AppData[0] = ((tempr * 10) >> 8) & 0xFF;
//		AppData[1] = (tempr * 10) & 0xFF;
		AppData[0] = tempr;

// 2 bytes For Cayenne
//		AppData[4] = 0x02; // Channel
//		AppData[5] = 0x03; // Type Analog Output
		bat = BoardBatteryMeasureVolage();
		AppData[1] = ((bat / 10) >> 8) & 0xFF;
		AppData[2] = (bat / 10) & 0xFF;

/*		AppData[8] = 0x03; // Channel
		AppData[9] = 0x068; // Type humidity
		AppData[10] = (humi * 2) & 0xFF;
*/
		AppDataSize = 3;
//		printf("[Debug]: tempr: %d Bat: %d mV Humi: %d\r\n", tempr, bat, humi);
	}
		break;
...
//			TimerStart( &Led1Timer );
		...
		case DEVICE_STATE_SEND: {
			if (NextTx == true) {
				...
				if (GPS_GETFAIL == SUCCESS) { GPS_GETFAIL = FAIL; }
				else { NextTx = SendFrame(); }

//				AppPort++;
//				if (AppPort >= 5) {	AppPort = 2; }
			}
			if (ComplianceTest.Running == true) {
